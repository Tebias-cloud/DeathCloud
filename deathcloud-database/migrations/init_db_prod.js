...
async function run() {
  ...
  await pool.query(`DROP SCHEMA IF EXISTS runner CASCADE;`);
  await pool.query(`DROP SCHEMA IF EXISTS skies CASCADE;`);
  await pool.query(`DROP SCHEMA IF EXISTS game2d CASCADE;`);
  
  // Borrar tablas explícitamente en caso de que estén en otro esquema en el search_path
  const tablesToDrop = ['tickets', 'amigos', 'mensajes', 'usuarios', 'store_items', 'news_articles', 'games_catalog'];
  for (let t of tablesToDrop) {
